# Complete DevSecOps CI/CD Pipeline | Amazon EKS + Jenkins + Trivy + SonarQube + ECR + GitHub
...
In this Project, we build an end-to-end **production-grade DevSecOps pipeline** that integrates secure coding, vulnerability scanning, container image hardening, artifact management, and automated Kubernetes deployment.
...
By going through this repo, you will understand how each stage contributes to a hardened delivery pipeline — from scanning code on commit to deploying a signed, validated image on a topology-aware Kubernetes cluster.
...
![alt text](images/ssh-ec2-connect.png)
```bash
ssh -i <your-key>.pem ubuntu@<jenkins-controller-public-ip>
```
![alt text](images/j-controller.png)
...
* Name it: **DevSecOps-CI-CD-Pipeline-Prod-Project**
* Visibility: **Private**
...
git remote add private-repo https://github.com/Hands-On-Project-Lab/DevSecOps-CI-CD-Pipeline-Prod-Project.git

# temporarily set the remote URL with the token so we can push securely
git remote set-url private-repo https://<TOKEN>@github.com/Hands-On-Project-Lab/DevSecOps-CI-CD-Pipeline-Prod-Project.git
...
git remote set-url private-repo https://github.com/Hands-On-Project-Lab/DevSecOps-CI-CD-Pipeline-Prod-Project.git
```
...
* Name → `DevSecOps-CI-CD-Pipeline-Prod`
* Type → **Pipeline**
...
* Name → `devsecops_ci_cd_pipeline_prod_project`
...
1. Open AWS Console → **Amazon ECR** → select repository `devsecops_ci_cd_pipeline_prod_project`.
...
```yaml
# file: k8s/devsecops-ci-cd-pipeline-prod-project-deploy.yaml
apiVersion: apps/v1
kind: Deployment
metadata:
  ...
  labels:
    app: devsecops-ci-cd-pipeline-prod-project
spec:
  replicas: 2
  selector:
    matchLabels:
  ...
  template:
    ...
    spec:
      ...
      containers:
        - name: devsecops-ci-cd-pipeline-prod-project
          image: 661676948454.dkr.ecr.ap-south-1.amazonaws.com/devsecops_ci_cd_pipeline_prod_project:latest
          ports:
            - containerPort: 8080
...
# file: k8s/devsecops-ci-cd-pipeline-prod-project-svc.yaml
apiVersion: v1
kind: Service
metadata:
  name: devsecops-ci-cd-pipeline-prod-project-svc
  namespace: devsecops-project
  labels:
    app: devsecops-ci-cd-pipeline-prod-project
spec:
...
```
![alt text](images/eks-clustre.png)

![alt text](images/eks.png)

![alt text](images/eks-ins1.png)
    ...
    "Statement": [
        {
            ...
            "Resource": "arn:aws:eks:ap-south-1:661676948454:cluster/*"
        }
    ]
}
```
...
![alt text](images/running-pods.png)

* Test with `http://<public-ip-of-instance-created-by-eks>:<exposed-nodeport>` -> http://65.0.91.246:31000/
